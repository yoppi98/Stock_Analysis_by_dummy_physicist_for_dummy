...
data = [go.Candlestick(x = plot_df.index, open = plot_df["Open"], 
        ...
        go.Bar(x=plot_df.index, y=plot_df["Volume"], name="Volume", yaxis="y2",marker=dict(color="lightgray"),
        opacity=0.7)
        ]

layout = {"height":800, "title":{"text":"{} {}".format(ticker, name), "x": 0.5},
          "xaxis": {"title": "Date", "rangeslider": {"visible": False} ,"type": "category","tickvals": plot_df.index[::2],
                    "anchor": "y2"},
          "yaxis1": { "domain":[.20, 1.0] ,"title": "Stock Price (Dollar)", "side":"left", "tickformat":","},
          "yaxis2": {"domain":[0.10, 0.20], "title": "Volume", "showgrid":True},
          "yaxis3": {"domain": [0, 0.10]},
